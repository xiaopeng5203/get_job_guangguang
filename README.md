...
本次更新后，Boss直聘自动化投递系统的AI打招呼功能有如下优化：

### 1. 综合岗位信息生成打招呼内容
- AI打招呼时，会同时参考岗位名称、岗位描述、岗位职责、岗位要求等所有信息，生成更贴合实际岗位的打招呼语。
- 无论是普通岗位还是推荐岗位，均会抓取岗位详情页的所有相关描述内容，传递给AI进行分析。

### 2. 推荐岗位与普通岗位均支持
- 推荐岗位流程和普通岗位流程都已支持AI智能打招呼。
- 推荐岗位流程调用AI时，会自动传递岗位名称作为岗位方向，确保AI生成内容的针对性。

### 3. 打招呼风格优化
- AI打招呼风格为"活泼但不过于随意，保留一定正式感"，适合直接发给HR。
- 你可以在`config.yaml`中自定义prompt模板，灵活调整风格。

### 4. 配置文件prompt优化
- `src/main/resources/config.yaml`中的`ai.prompt`已优化为：

```yaml
prompt: |
  我目前在找工作，%s。我的目标岗位方向是【%s】。
  当前需要投递的岗位名称是【%s】，岗位描述/职责/要求如下：【%s】。
  请你判断这个岗位和我的期望、经历是否基本匹配（只要有一定相关性即可），
  如果匹配，请用活泼但不过于随意的语气，写一段简洁有感染力的打招呼内容，突出我的优势和岗位的契合点，适合直接发给HR。
  如果完全不相关，直接返回false，不要多余解释。
  下面是我自己写的招呼语供你参考：【%s】。你可以结合岗位实际情况适当调整。
```

### 5. 关键实现细节
- 岗位详情页抓取时，会将所有"岗位描述/职责/要求"相关内容拼接后赋值给`job.setJobKeywordTag`，确保AI能获取到完整信息。
- 推荐岗位流程调用AI时，第三个参数为岗位名称，保证prompt的"岗位方向"字段有内容。
- 这样无论普通岗位还是推荐岗位，AI打招呼都能用上岗位名称和所有描述信息，风格也会如你所愿。

---
如需调整AI打招呼风格或内容，只需修改`config.yaml`中的`ai.prompt`即可。
