...
## 推荐岗位与普通岗位关键词过滤逻辑优化（2025-05-22）

### 推荐岗位Tab投递逻辑优化
- **推荐岗位Tab优先级可配置**：通过 `config.yaml` 的 `recommendTabPriority` 配置项，可以手动指定推荐Tab的优先点击顺序，支持精确到"岗位名称(城市)"的Tab（如 `售前技术支持(厦门)`），也支持模糊匹配（如只写 `售前技术支持`）。
- **推荐Tab下岗位只看岗位名称关键词**：在投递推荐Tab下的岗位时，只判断岗位名称是否包含配置文件中的任意关键词。只有名称包含关键词的岗位，才会进入详情页并进行后续投递动作，不会再点进无关岗位详情页，大幅提升效率。
- **推荐Tab与普通岗位完全分离**：推荐Tab下的岗位不会再判断描述/职责是否包含关键词，只看岗位名称，逻辑更清晰、可控。

### 普通岗位投递逻辑优化
- **普通岗位多维关键词判断**：在普通岗位（非推荐Tab）投递时，会判断岗位名称、描述或职责三者之一是否包含任意关键词，只要三者之一包含关键词，就会进入详情页并进行投递。
- **普通岗位与推荐岗位判断方式完全区分**：推荐岗位只看名称，普通岗位看名称+描述+职责，互不影响，逻辑独立。

### 代码健壮性与可维护性提升
- **消除变量重复声明**：彻底清理了推荐岗位投递流程中关于 `containsKeyword` 的重复声明和多余判断，避免了编译报错和潜在逻辑混乱。
- **前置过滤，极致高效**：推荐岗位的关键词过滤在进入详情页前就完成，普通岗位的多维过滤也在进入详情页前完成，极大提升了整体运行效率。

### 配置示例
```yaml
boss:
  recommendTabPriority: [ "售前技术支持(厦门)", "售前技术支持(深圳)", "项目经理(厦门)" ]
  keywords: [ "售前技术支持", "项目经理", "解决方案" ]
  # 其他配置...
```
